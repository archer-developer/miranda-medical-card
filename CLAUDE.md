...
- **`cmd/miranda-medical-card`** — the only place production wiring happens (`main.go`'s `run()`):
  ...
  (full LLM request/response trace) and `logs/miranda-medical-card.log` (the full app log — every level
  from Debug up, not just DEBUG-tagged lines; named for what it holds, not "debug.log", since it's the
  one place to look for a WARN/ERROR too) are both only written when `logging.level: debug`, and both
  rotate by size via `gopkg.in/natefinch/lumberjack.v2` (`rotatingLogFile`,
  `logging.max_size_mb`/`max_backups`/`max_age_days` in config). The app log file is a superset of what
  ...
  chattier than miranda's own. An earlier version (when the file was still called `debug.log`) routed
  each record to exactly one destination by level (DEBUG ⇒ file only, everything else ⇒ stdout only), so
  a WARN/ERROR — a failed LLM call, a provider escalation, `"upload_document failed"` — never reached the
  file at all; only `journalctl` had it, so the file alone could go quiet mid-request with no visible
  reason why (found in production 2026-08-27, see git log around `teeHandler`).
