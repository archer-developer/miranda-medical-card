...
| Маршрутизация моделей | `internal/config.LLMConfig` — `ocr_provider` (Stage 1) и `extraction_provider` (Stage 2a/2b и всё Structured-подобное в Document Pipeline) конфигурируются по имени независимо друг от друга и от `agent_provider` (agent loop, см. `03-knowledge-providers.md`) — могут называть один и тот же провайдер или разные |
| Автоматическая эскалация | Два независимых механизма — см. "Tool-based эскалация" и "Конфигурация провайдеров" ниже: `router.Router` (reliability fallback + tool-based эскалация) для `agent_provider`, caller-side проверка качества результата только для OCR/Structured Extraction (`ocr_provider`/`extraction_provider`) |
...
| Streaming | `llm.Provider.Chat` — потоковый по умолчанию; используется для OCR (Stage 1, `ocr_provider`) и, через `router.Router`, для каждого хода Agent Loop |
...
- **`ocr_provider`/`extraction_provider` (Document Pipeline) — caller-side, по качеству результата, каждый независимо.** Для `extraction_provider`: если собственные попытки Structured Extraction исчерпаны и результат всё ещё выглядит подозрительно пустым (`internal/extraction.isSuspiciouslyEmpty`, см. `02-processing-pipeline.md` §5), делается одна попытка против `escalation.target_provider` — другой модели/провайдера, которая может не разделять то, из-за чего основная не справилась (`internal/extraction.StructuredWithRetry`). Для `ocr_provider`: любая жёсткая ошибка OCR retry'ится один раз против `escalation.target_provider` (`internal/extraction.Extract`'s `ocrProvider`/`ocrEscalate`). Ни то, ни другое не проходит через `router.Router`: оба реагируют на конкретный, специфичный для своего этапа сигнал (для Structured Extraction — не ошибка, а структурно валидный, но подозрительно пустой результат; то, что `router.Router` в принципе не умеет замечать, поскольку не заглядывает в содержимое успешного ответа), а не на общую "провайдер недоступен" reliability-логику. Только `target_provider` читается для этих двух ролей; `tool_name`/`description` игнорируются.
...
Провайдер с пустым `escalation.tool_name` (как сегодняшние `ocr_provider`/`extraction_provider` в примере конфигурации) остаётся вне tool-based эскалации `router.Router`, даже будучи одним из провайдеров, которыми обёрнут `router.Router` — это то, что сохраняет его прежнее поведение неизменным при переходе на общий `router.Router`.
