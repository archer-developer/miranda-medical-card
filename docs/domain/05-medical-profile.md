...
- Пересобирается **полностью** (не инкрементально) после каждого успешного `upload_document` и `delete_document` для этого пользователя — см. `../architecture/02-processing-pipeline.md` §8.
