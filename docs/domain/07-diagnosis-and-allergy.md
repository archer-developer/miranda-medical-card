...
переиспользуется тот же код, что и для `PlannedAction`), заякоренная на `diagnosedAt` — которая сама к этому моменту
уже гарантированно заполнена (см. описание поля выше), в т.ч. подстановкой даты документа.
...
своего дефолтного состояния, а не пытаются сопоставить старую и новую запись по полному содержимому (код, диапазон
дат и т.п.), поскольку результат Structured Extraction не гарантированно одинаков между запусками (см.
`../architecture/02-processing-pipeline.md` §11).

Один частичный шаг навстречу такому сопоставлению всё же есть: `ReplaceForDocument` не вставляет свежую запись,
