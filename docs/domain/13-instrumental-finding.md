...
| `value` | number | ❌ | Числовое значение, если параметр количественный. |
| `unit` | string | ❌ | Единица измерения (`мм`, `см`, `см/с`), если `value` задано. |
| `qualitativeValue` | string | ❌ | Описательное значение, если параметр не числовой (например "однородная", "не расширена"). Ровно одно из `value`/`qualitativeValue` обычно заполнено, но оба могут быть пустыми, если параметр упомянут без конкретного значения. |
