...
| `unit` | string | Единица измерения (`U/L`, `mmol/L`, ...). |
...
| Поле | Тип | Описание |
|------|-----|----------|
| `system` | enum (`icd10`, ...) | Система кодирования. Единственное поддерживаемое значение на текущий момент — `icd10`; поле существует для будущего расширения (SNOMED CT и др., см. `../architecture/04-search.md` §24), а не потому что несколько систем уже поддержаны. |
| `code` | string | Сам код (например `I10`). |

Value Object не имеет собственного текстового описания — человекочитаемое название хранится отдельно в `Diagnosis.name`, поскольку ровно одно название на диагноз нужно всегда, а код — не всегда доступен.
