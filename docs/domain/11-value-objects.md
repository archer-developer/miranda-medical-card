...
| Поле | Тип | Описание |
|------|-----|----------|
| `value` | number (опционально) | Измеренное значение, как в документе — только для количественных показателей. |
| `qualitativeValue` | string (опционально) | Описательное или категориальное значение, если показатель не числовой (например группа крови `A(II) Rh+`, результат теста `положительно`/`отрицательно`). Ровно одно из `value`/`qualitativeValue` обычно заполнено, но оба могут быть пустыми. Тот же принцип, что у `InstrumentalFinding.qualitativeValue` (`13-instrumental-finding.md` §2). |
| `unit` | string | Единица измерения, как в документе (`U/L`, `mmol/L`, ...) — заполнено только при количественном `value`. |
| `normalizedValue` | number (опционально) | `value`, приведённое к каноничной для этого показателя единице — см. `09-lab-result-and-vital-sign.md` §7. Пусто, если единица не распознана как приводимая, для показателя ещё не установлена каноничная единица, или показатель качественный (`unit` пусто). |
...
| `flag` | enum (`normal`, `low`, `high`) | Вычисляется относительно `referenceLow`/`referenceHigh`, если оба заданы; иначе не заполняется — сервис не подставляет собственные референсные значения (см. `09-lab-result-and-vital-sign.md` §4). Для качественных показателей референсный диапазон не имеет смысла и не заполняется, поэтому `flag` тоже не вычисляется. |
