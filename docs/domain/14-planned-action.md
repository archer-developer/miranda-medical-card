...
- Пункт с `sourceType: document` принадлежит ровно одному документу, но, в отличие от всех
  ...
  *другого* документа — поэтому переобработка документа-источника не реплейсит пункты плана вслепую
  ...
  и новую запись по содержимому (`type` + канонизированное имя) — результат Structured Extraction не
  гарантированно одинаков между запусками (см. `../architecture/02-processing-pipeline.md` §11), так
  что содержимое ненадёжно как идентификатор; статус — надёжен. Цена: повторная обработка документа
  может вставить рядом с уже `completed`-записью свежую `pending`-запись с похожим текстом того же
  пункта — редкий, но безобидный побочный эффект против тихой потери состояния или хрупкого
  сопоставления.
...
ReplaceForSource(sourceType, sourceId string, actions []PlannedAction) error  // документы: удаляет только pending, дедуп между источниками, см. §5
