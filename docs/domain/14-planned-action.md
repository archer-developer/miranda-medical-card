...
- Пункт с `sourceType: document` принадлежит ровно одному документу, но, в отличие от всех
  ...
  *другого* документа — поэтому переобработка документа-источника не реплейсит пункты плана
  вслепую (delete+reinsert), а **сверяет** их по ключу (`type` + канонизированное имя): найденный
  по ключу пункт сохраняет `id`/`status`/`matched*`, обновляются только описание и диапазон дат;
  пункт, чей ключ исчез, удаляется только если он ещё `pending` — уже `completed` запись не
  трогается.
