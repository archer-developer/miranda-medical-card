...
По умолчанию команда только читает уже сохранённый снимок `MedicalProfile`. С флагом `--rebuild`
она сначала пересобирает профиль по уже сохранённым данным (тот же шаг, который сервис и так
выполняет автоматически после каждого `upload_document`/`reprocess_document`/`log_event`, см.
`../domain/05-medical-profile.md` §4) и печатает уже пересобранный результат — без повторного
OCR/Structured Extraction над документами пользователя.

```bash
medical-dev profile --user alex --rebuild
```

В отличие от базового вызова, это операция записи — перезаписывает сохранённый `MedicalProfile`
пользователя (см. §2 "любые операции, изменяющие данные, должны быть явно указаны пользователем" —
явный флаг здесь и есть такое указание). Полезно после изменения логики агрегации Profile
(например, добавления нового поля), чтобы обновить снимки уже существующих пользователей, не
трогая исходные документы.
