...
// TestTeeHandler_WarnReachesBothStdoutAndDebugFile reproduces a production
// bug (2026-08-27): the old levelSplitHandler routed each record to exactly
// one destination by level (DEBUG => file only, INFO+ => stdout only), so a
// WARN/ERROR — e.g. an OCR provider failing over, or "upload_document
// failed" — never appeared in debug.log at all, only in the stdout/journal
// stream. Debugging a stalled request by reading debug.log alone gave no
// indication anything had gone wrong. teeHandler fans every record out to
// both handlers (each still gated by its own configured level), so
// debug.log ends up a superset of what stdout sees, not a level-exclusive
// subset of it — matching miranda's own cmd/miranda/main.go, which writes
// one io.MultiWriter'd handler instead of splitting by level.
func TestTeeHandler_WarnReachesBothStdoutAndDebugFile(t *testing.T) {
	...
	logger := slog.New(&teeHandler{stdout: stdoutHandler, debugFile: fileHandler})
	...
	require.Contains(t, fileOut, "process start", "debug.log must keep receiving DEBUG-level trace lines")
	require.Contains(t, fileOut, "ocr failed", "debug.log must also receive WARN/ERROR lines, not just DEBUG ones")
