...
4. If the note describes something the user needs to do in the future (a follow-up test, a vaccination, a visit) — e.g. "нужно сделать прививку от бешенства в течение полугода" or, with no stated timeframe, "надо сдать анализ на глюкозу" — extract it as plannedAction, independent of category, same as medicationIntake. If a rough timeframe is stated, express it as a relative amount + unit (dueAmountMin/dueAmountMax/dueUnit), never as a calendar date you compute yourself — you don't reliably know today's date, only the offset the user stated. If no timeframe is stated, omit dueAmountMin/dueAmountMax/dueUnit entirely — the plannedAction is still valid, just with no due date.
