...
func TestCompletePlannedAction_HappyPath(t *testing.T) {
	ctx := context.Background()
	s, fs := newTestBackend(t)
	planRepo := storage.NewPlannedActionRepository(s)
	_, err := planRepo.Add(ctx, normalization.PlannedAction{
	...
	completed, err := p.CompletePlannedAction(ctx, "user1", "я сделал прививку от бешенства")
	require.NoError(t, err)
	require.Equal(t, rabies.ID, completed.ID)
	require.Equal(t, normalization.PlannedActionStatusCompleted, completed.Status)
	require.NotNil(t, completed.MatchedAt)

	all, err := planRepo.ListByUser(ctx, "user1")
	require.NoError(t, err)
	for _, a := range all {
		if a.ID == rabies.ID {
			require.Equal(t, normalization.PlannedActionStatusCompleted, a.Status)
			require.Empty(t, a.MatchedDocumentID, "manual completion has no closing document")
		} else {
			require.Equal(t, normalization.PlannedActionStatusPending, a.Status, "the other pending action must be untouched")
		}
	}
...
func TestCompletePlannedAction_NoPendingActionsAtAll(t *testing.T) {
	ctx := context.Background()
	s, fs := newTestBackend(t)
	p := pipeline.New(llmtest.New("fake"), nil, llmtest.New("fake"), nil, llmtest.NewFakeEmbedder([]float32{0.1, 0.2}), "fake", "fake-model", fs, s, nil, nil)

	_, err := p.CompletePlannedAction(ctx, "user1", "я сделал что угодно")
	require.Error(t, err)
	var notFound *pipeline.PlannedActionNotFoundError
	require.ErrorAs(t, err, &notFound)
	require.Empty(t, notFound.PendingDescriptions)
...
func TestCompletePlannedAction_NoConfidentMatchListsPendingDescriptions(t *testing.T) {
	ctx := context.Background()
	s, fs := newTestBackend(t)
	planRepo := storage.NewPlannedActionRepository(s)
	_, err := planRepo.Add(ctx, normalization.PlannedAction{
	...
	_, err = p.CompletePlannedAction(ctx, "user1", "я сделал что-то непонятное")
	require.Error(t, err)
	var notFound *pipeline.PlannedActionNotFoundError
	require.ErrorAs(t, err, &notFound)
	require.Equal(t, []string{"Повторный анализ глюкозы"}, notFound.PendingDescriptions)
...
func TestCompletePlannedAction_EmptyTextRejected(t *testing.T) {
	ctx := context.Background()
	s, fs := newTestBackend(t)
	p := pipeline.New(llmtest.New("fake"), nil, llmtest.New("fake"), nil, llmtest.NewFakeEmbedder([]float32{0.1, 0.2}), "fake", "fake-model", fs, s, nil, nil)

	_, err := p.CompletePlannedAction(ctx, "user1", "   ")
	require.Error(t, err)
	var notFound *pipeline.PlannedActionNotFoundError
	require.NotErrorAs(t, err, &notFound, "empty text must be its own plain error, not PlannedActionNotFoundError")
}
