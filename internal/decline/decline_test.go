...
func TestMatch_EmptyCandidatesReturnsEmptyWithoutCallingProvider(t *testing.T) {
	...
	got, err := decline.Match(context.Background(), provider, "отмени прививку от бешенства", nil)
...
func TestMatch_ExactCandidateReturned(t *testing.T) {
	provider := llmtest.New("fake").WithStructured(llmtest.StructuredResponse{
		JSON: json.RawMessage(`{"plannedActionId":"plan_2"}`),
	})

	got, err := decline.Match(context.Background(), provider, "отмени прививку от бешенства", []decline.Candidate{
...
func TestMatch_NoConfidentMatchReturnsEmpty(t *testing.T) {
	...
	got, err := decline.Match(context.Background(), provider, "отмени что-то непонятное", []decline.Candidate{
		{ID: "plan_1", Description: "Повторный анализ глюкозы", Type: "lab_test"},
	})
	require.NoError(t, err)
	require.Empty(t, got)
...
func TestMatch_ProviderErrorIsPropagated(t *testing.T) {
	...
	_, err := decline.Match(context.Background(), provider, "text", []decline.Candidate{{ID: "plan_1", Description: "x"}})
	require.Error(t, err)
}
